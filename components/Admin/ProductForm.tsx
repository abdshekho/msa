...
import TableEditor from './TableEditor'; // Import your existing TableEditor component
...
export default function ProductForm({ productId }: ProductFormProps) {
    ...
    const [tableData, setTableData] = useState({
        headers: ['Model', "Product id1", "Product id2", "Product id3"], rows: [
            {
        ...
        if (productId) {
            const fetchProduct = async () => {
                try {
                    ...
                    console.log('🚀 ~ ProductForm.tsx ~ fetchProduct ~ data:', data);
    ...
    useEffect(() => {
        ...
        const fetchParentCategories = async () => {
            try {
                // Use the query parameter to get categories where parentId is not null
                // We can use 'notNull=true' as a custom parameter
                const response = await fetch('/api/categories?notNull=true');
    ...
    const handleFileArrayChange = async (name: string, index: number, e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            const file = e.target.files[0];
            const formData = new FormData();
            formData.append('image', file);
